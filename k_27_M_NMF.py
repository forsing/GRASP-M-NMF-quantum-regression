...
# Varijanta: M-NMF (Modularity NMF) – matrica susedstva + modularnost, ne-negativna faktorizacija

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
M-NMF = A + beta*B (B = modularity), NMF: M ≈ W*H, embedding = W
"""
...
MNMF_BETA = 0.2  # skalirajuca konstanta
MNMF_ITER = 80   # broj iteracija
MNMF_EPS = 1e-10 # epsilon
...
def compute_mnmf_embeddings(M, k=EMBED_DIM):
    ...
    M_mat = np.maximum(A + MNMF_BETA * B, 0.0) + 1e-8

    W = np.abs(np.random.randn(39, k)) * 0.1 + 0.01
    H = np.abs(np.random.randn(k, 39)) * 0.1 + 0.01

    for _ in range(MNMF_ITER):
        W = W * (M_mat @ H.T) / (W @ H @ H.T + MNMF_EPS)
        H = H * (W.T @ M_mat) / (W.T @ W @ H + MNMF_EPS)
...
def main():
    ...
    emb = compute_mnmf_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (M-NMF + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (M-NMF + kvantna regresija, deterministički, strukturno):
    (8, 11, 22, 23, 26, 28, 33)

    Score: 0.5100135535855859
    """
...
"""
M-NMF (Modularity NMF):

B = modularna matrica: B_ij = A_ij − k_i·k_j/(2m).

M_mat = max(A + β·B, 0) + ε (ne-negativna, β=0.2).

NMF: M_mat ≈ W·H (rang k); 
multiplikativna pravila za W (39×k) i H (k×39), 
80 iteracija; W i H inicijalizovani nenegativno (seed=SEED).
emb = W, zatim normalizacija u [0, π].
"""
